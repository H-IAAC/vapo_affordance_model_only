...
from vapo.agent.core.utils import get_nets, soft_update, tt
...
class SAC:
    def __init__(
        ...
        if wandb_login and not resume:
            ...
            wandb.init(name=model_name, config=config, id=id, resume="allow", **wandb_login)
        else:
            id = 0
        ...
        # Replay buffer
        _cnn_policy_cond = ["gripper_depth_obs", "gripper_img_obs", "static_depth_obs", "static_img_obs"]
        _img_obs = False
        obs_space = env.observation_space
        if isinstance(obs_space, gym.spaces.Dict) and any([x in _cnn_policy_cond for x in obs_space]):
            _img_obs = True
        ...
        if isinstance(alpha, str):  # auto
            ...
            self.log_ent_coef = torch.tensor(np.log(init_temp), requires_grad=True, device="cuda")  # init value
        ...
        policy_net, critic_net, obs_space, action_dim = get_nets(
            _img_obs, obs_space, env.action_space, self.log, actor_net, critic_net
        )
        self._pi = policy_net(obs_space, action_dim, action_space=env.action_space, **net_cfg).cuda()
    ...
    def _update(self, td_target, batch_states, batch_actions):
        ...
        predicted_actions, log_probs = self._pi.act(_batch_states, deterministic=False, reparametrize=True)
        critic_value = torch.min(self._q1(_batch_states, predicted_actions), self._q2(_batch_states, predicted_actions))
    ...
    def training_step(self, s, ts, ep_return, ep_length):
        ...
        new_data = {}
        if self._replay_buffer.__len__() >= self.batch_size and not done and ts > self.learning_starts:
            ...
            with torch.no_grad():
                batch_next_states = self.env.transform_obs(batch_next_states, "train")
                next_actions, log_probs = self._pi.act(batch_next_states, deterministic=False, reparametrize=False)

                target_qvalue = torch.min(
                    self._q1_target(batch_next_states, next_actions), self._q2_target(batch_next_states, next_actions)
                )
            ...
            # ----------------  Networks update -------------#
            new_data = self._update(td_target, batch_states, batch_actions)
        return s, done, success, ep_return, ep_length, new_data.copy(), info

    def _on_train_ep_end(self, ts, episode, total_ts, best_return, episode_length, episode_return, success, plot_data):

        print_str = (
            "[%d] %s, " % (episode, self.env.target)
        ...
        write_dict = {"timesteps": ts, "episode": episode}
        ...
        if episode_return >= best_return:
            self.log.info("[%d] New best train ep. return!%.3f" % (episode, episode_return))
            self.save(self.trained_path + "best_train.pth")
            best_return = episode_return

        if last_n_train_mean_success >= self.last_n_train_mean_success:
            self.log.info(
                "[%d] New best train ep. success: %.3f over last %d ep !"
    ...
    def _eval_and_log(self, t, episode, most_tasks, best_eval_return, n_eval_ep, max_ep_length, eval_all_objs=False):
        # If all objs are already on the table
        if eval_all_objs and self.eval_env.rand_positions is None:
            return
        ...
        if eval_all_objs:
            ...
            success_lst, objs_success = self.eval_all_objs(self.eval_env, max_ep_length)
            ...
            mean_return, mean_length, success_lst, objs_success = self.evaluate(
                self.eval_env, max_ep_length, n_episodes=n_eval_ep
            )
            write_dict.update(
                {
        ...
        if self.sim:
            if self.eval_env.task == "pickup" and self.eval_env.rand_positions:
    ...
    def save(self, path):
        ...
        if self._save_replay_buffer:
            self._replay_buffer.save(os.path.join(self.save_dir, "replay_buffer"))
    ...
    def load(self, path, resume_training=False):
        if os.path.isfile(path):
            ...
            if resume_training:
                ...
                wandb.init(id=self.wandb_id, resume="must", config=config, **self.wandb_login)
            ...
            raise TypeError("Model path does not exist: %s \n" % os.path.abspath(path))
